...
public class MM_Logic : MonoBehaviour {
    ...
    public AudioClip[] ScenemanagerSounds;                    //SceneManagerSound - plays the sound when level loaded or reloaded for example wind ambient sound | Changed to array new in v.2.0
    ...
    public void GameModeSection(int GM) //where GM - Gamemode number new in v2.0
    {
        int levelnum = 0;
    ...
    void LevelSection(int levelnum,int gamemode) //levelNum set from GameModeSection new in v.2.0
    {
        if (!clicked)
        {
            ...
            if (levelnum == 1)
            {
                ...
                CamHolder.GetComponent<Animation>().Play("MM_Way01"); //Changed to legacy animation new in v.2.0
            }

            if (levelnum == 2)
            {
                Scenemanager.GetComponent<SceneGM>().GameMode = gamemode; //reseting GameMode to default
                CamHolder.GetComponent<Animation>().Play("MM_Way02"); //Changed to legacy animation new in v.2.0
            }
